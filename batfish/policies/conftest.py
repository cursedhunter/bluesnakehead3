...
def pytest_addoption(parser):
    ...
    parser.addoption("--batfish-host", action="store", required=True,
                     help="Host where Batfish service can be found")


@pytest.fixture(scope="session")
def bf(request):
    host = request.config.getoption("--batfish-host")

    try:
        bf = Session.get('bfe', host=host)
        os.environ["SESSION_TYPE"] = 'bfe'
    except:
        bf = Session.get('bf', host=host)
